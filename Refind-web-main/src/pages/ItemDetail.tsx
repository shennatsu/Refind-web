...
const ReportModal = ({
  ...
  e.preventDefault();
  const reason = (document.querySelector('input[name="report_reason"]:checked') as HTMLInputElement)?.value;
  const details = (document.querySelector('textarea') as HTMLTextAreaElement)?.value;
  ...
  try {
    ...
    const res = await fetch('http://localhost:5000/api/reports', {
      ...
      body: JSON.stringify({ postId: itemId, reason, details })
    });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        ...
        <p className="text-gray-600 mb-6">
          Mengapa Anda melaporkan postingan ini? Pilih salah satu alasan di bawah.
        </p>
        <form onSubmit={handleReportSubmit}>
          <div className="space-y-4">
            {[
              "Spam atau iklan",
              "Indikasi Penipuan (Scam)",
              "Konten Tidak Pantas",
...
const ItemDetail = () => {
  ...
  if (!item) return <div className="text-center py-20">Item tidak ditemukan.</div>;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1">
            <img
              src={
                item.imageUrl ||
                `https://placehold.co/600x400/EEE/000?text=${item.category}`
              }
              alt={item.title}
          ...
          <div className="md:col-span-2">
            ...
            <div className="space-y-4 text-lg">
              ...
              <div>
                ...
                <p className="text-blue-600">#{item.category || "Umum"}</p>
              </div>
            </div>
            ...
            <div className="mt-8 p-4 bg-gray-50 rounded-lg flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <span className="font-semibold">
                  {item.owner?.name || "Pengguna"}
                </span>
              </div>
              <div className="flex space-x-3">
                <button
                  onClick={() => navigate(`/messages/${item.owner?._id}`)}
                  className="bg-gray-200 px-4 py-2 rounded-md hover:bg-gray-300 flex items-center gap-2"
                >
